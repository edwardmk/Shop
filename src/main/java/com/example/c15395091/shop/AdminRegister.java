    ...
    public void RegisterUser() {
        ...
        if (!rUsername.equals("") && !rEmail.equals("") && !rPword.equals("") && !rConfirmPword.equals("")) {
            if (rPword.equals(rConfirmPword)) {
                mAuth.createUserWithEmailAndPassword(rEmail, rPword).addOnCompleteListener(AdminRegister.this, new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (!task.isSuccessful()) {
                            ...
                            User userInfo = new UserBuilder().setUsername(rUsername).setEmail(rEmail).setShippingAddress(rAddress).setAdmin("true").createUser();
                            myRef.child("users").child(userID).setValue(userInfo);

                            Intent i = new Intent(AdminRegister.this, HomePage.class);
                            startActivity(i);
                            finish();
